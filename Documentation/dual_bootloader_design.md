...
> **Status:** design proposal. Not implemented. Implementation is **gated on the CMC
> moving to a larger STM32 in the same family** (current STM32G431RB has 128 KB flash;
> the design needs ~150 KB minimum for bootloader + app, comfortably more if A/B
> redundancy is adopted). The motor-MCU side has no such constraint and could be
> built independently if/when prioritised.
>
